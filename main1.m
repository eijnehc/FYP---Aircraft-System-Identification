%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This is a similiar program to main but it is you to compare the time 
% histroy plots of the all the parameter estimation methods
% There are two types of inputs you can choose to deflect the elevators
% Next,there is an option to add noise to the system
% Lastly, choose either of the 5 system identification methods
...
%     eta = 0.1; % Learning rate for LMS
%     sacd = zeros(4,5); % Initial weights of LMS
%     
%     for n = 1:N
%         yc = (sacd)*X'; % Output of LMS                           
%         ec = Y - yc'; % Instantaneous error of LMS 
%         sacd = sacd +  eta * ec' * X; % Weight update rule of LMS
%     end
...
sys_hat = ss(Ahat1,Bhat1,C,[]); % continuous estimated state-space model 
delta_xhat1 = lsim(sys_hat,delta_u,tspan); 

sys_hat = ss(Ahat2,Bhat2,C,[]); % continuous estimated state-space model 
delta_xhat2 = lsim(sys_hat,delta_u,tspan); 

sys_hat = ss(Ahat3,Bhat3,C,[]); % continuous estimated state-space model 
delta_xhat3 = lsim(sys_hat,delta_u,tspan); 

sys_hat = ss(Ahat4,Bhat4,C,[]); % continuous estimated state-space model 
delta_xhat4 = lsim(sys_hat,delta_u,tspan); 
...
% % Goodness of fit
% cost_func = 'NRMSE';
% fit = goodnessOfFit(delta_xhat*r2d,delta_xsim*r2d,cost_func)*100;
% fit = round(fit,2);
% 
% str = 'Goodness of Fit';
% idx = ~isstrprop(str,'wspace'); % detect spaces
...
plot_result1(tspan,delta_xsim,delta_xhat1,delta_xhat2,delta_xhat3,delta_xhat4)
